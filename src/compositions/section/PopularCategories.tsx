...
export const PopularCategoriesSection = ({
  ...
  return (
    <section
      ...
      className={cn('space-y-4 -mx-4 sm:-mx-8 xl:mx-0', className)}
      {...props}
    >
      <h2 className="text-sm font-bold px-4 sm:px-8 xl:px-0">Popular</h2>
      <div className={categoryVariants({ component: 'list', layout })}>
        {categories.map((cat) => (
...
const categoryVariants = cva('', {
  variants: {
    component: {
      list: 'flex flex-row xl:flex-col gap-4 cursor-pointer snap-x snap-mandatory overflow-x-auto no-scrollbar',
  ...
  compoundVariants: [
    ...
    },
    {
      component: 'list',
      layout: 'horizontal',
      className: 'flex flex-col items-center min-w-[90px] px-4 sm:px-8 py-4',
    },
    {
      component: 'item',
      layout: 'vertical',

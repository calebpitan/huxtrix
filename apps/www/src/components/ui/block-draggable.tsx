...
export const BlockDraggable: RenderNodeWrapper = (props) => {
  ...
  const BlockDraggableRenderNodeWrapper = (props: PlateElementProps) => <Draggable {...props} />

  return BlockDraggableRenderNodeWrapper
}
...
function Draggable(props: PlateElementProps) {
  ...
  const forkedHandleRef = useForkRef(handleRef, dragHandleRef)

  const calibrateDragHandle = React.useCallback(() => {
    if (isDragging) return

    const drgaHandle = dragHandleRef.current
    const preview = previewRef.current
    const previewContent = preview?.firstElementChild as HTMLElement | null | undefined

    if (!drgaHandle || !preview) {
      return setDragButtonTop(calcDragButtonTop(editor, element))
    }

    const diff = preview.clientHeight - (previewContent?.clientHeight ?? 0)
    const diff2 = previewContent
      ? previewContent.clientHeight - drgaHandle.clientHeight
      : drgaHandle.clientHeight

    setDragButtonTop(diff + diff2 / 2)
  }, [editor, element, isDragging, previewRef])
  ...
  return (
    <div
      ...
      onMouseEnter={calibrateDragHandle}
    >
      {!isInTable && (
        <Gutter>
          <div className={cn('slate-blockToolbarWrapper', 'flex h-[1.5em]', isInColumn && 'h-4')}>
            <div
              className={cn(
                'slate-blockToolbar w-4.5 relative',
...
function Gutter({ children, className, ...props }: React.ComponentProps<'div'>) {
  ...
  return (
    <div
      ...
      className={cn(
        'slate-gutterLeft',
        'z-45 absolute top-0 flex h-full -translate-x-full cursor-text hover:opacity-100 sm:opacity-0',
